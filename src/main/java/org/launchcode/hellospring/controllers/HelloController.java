...
@ResponseBody //Since each method within the class uses @ResponseBody, we can put it above the class instead of above each class. This annotation tells Spring Boot that this method will return a plain text HTTP response. Doesn't use HTML.
@RequestMapping("hello") //Says every single method within class should begin with /hello . So any info we put in after it, like /goodbye, now lives at /hello/goodbye
...
//    @GetMapping("hello")
    ...
    @GetMapping("{name}") //put '/' after hello and variable 'name' in curly brackets to signify you want this to accept piece of data in path
